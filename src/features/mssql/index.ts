  ...
  return new Promise((resolve, reject) => {
    conn.execute({
      ...
      complete: (err, stmt, rows) => {
        ...
        try {
          // getNumUpdatedRows() returns number of rows affected by DML
...
async function mergeCsvIntoTable(conn: Connection, tableName: string, csvFilePath: string): Promise<number>  {
  ...
  // Upload CSV file into Snowflake stage
  await uploadFileToStage(conn, csvFilePath, stage);
  ...
  // Capture rows affected by the MERGE statement
  ...
  try {
    ...
    await runWithConcurrencyLimit(tables, 10, async (table) => {
      ...
      try {
        ...
        const rowsAffected = await mergeCsvIntoTable(conn, snowflakeTableName, csvPath);
